...
class DeliverymanRegistrationController extends GetxController implements GetxService {
  ...
  XFile? _aadhaarImage;
  XFile? get aadhaarImage => _aadhaarImage;

  XFile? _panCardImage;
  XFile? get panCardImage => _panCardImage;
  ...
  // void pickDmImage(ImageType type, bool isRemove) async {
  //   if (isRemove) {
  //     switch (type) {
  //       case ImageType.profilePic:
  //         _pickedImage = null;
  //         break;
  //       case ImageType.aadhaar:
  //         _aadhaarImage = null;
  //         break;
  ...
  //           break;
  //         case ImageType.panCard:
  //           _panCardImage = picked;
  //           break;
  //         case ImageType.identity:
  //           if (_pickedIdentities.length < 6) {
  //             _pickedIdentities.add(picked);
  //           }
  //           break;
  //       }
  //     }
  ...
  Future<void> postRegistrationData() async {
    var uri = Uri.parse('https://smartvyapaar.com/node-api/delivery/:id/documents');
    var request = http.MultipartRequest('POST', uri);
    request.fields['aadhaar_number'] = aadhaarController.text;
    request.fields['pan_number'] = panCardController.text;
    ...
    if (_panCardImage != null) {
      request.files.add(await http.MultipartFile.fromPath('pan_image', _panCardImage!.path));
    }
    try {
