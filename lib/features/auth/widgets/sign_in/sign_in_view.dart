...
import 'package:sixam_mart/util/helper/rt_logger.dart';
...
class _SignInViewState extends State<SignInView> {
  ...
  Widget build(BuildContext context) {
    rtDebug("yash button press; ",tag: tag);
    return Form(
      key: _formKeyLogin,
      ...
      )//activeCentralizeLogin(Get.find<SplashController>().configModel!.centralizeLoginSetup!),
    );
  }
...
rtDebug("yash activeCentral call",tag: tag);
...
rtDebug("phone: $phone , number with contry: $numberWithCountryCode, phone valid; $phoneValid, numberWithCountryCode: $numberWithCountryCode  ",tag: tag);
    if(_formKeyLogin!.currentState!.validate()) {
      ...
      rtDebug("yash : else call in ",tag: tag);
    }
  }
